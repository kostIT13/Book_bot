...
async def process_continue_command(message: Message, book: dict, db: dict):
    ...
    await message.answer(
        text=text,
        reply_markup=create_pagination_keyboard(
             "backward", f"{db["users"][message.from_user.id]["page"]}/{len(book)}", "forward"   
        )
    )
...
async def process_forward_press(callback: CallbackQuery, book: dict, db:dict):
    ...
    if current_page < len(book):
        ...
        await callback.message.edit_text(
            text=text,
            reply_markup=create_pagination_keyboard(
                "backward", f"{current_page + 1}/{len(book)}", "forward"
            )
        )
...
async def process_bookmarks_press(callback: CallbackQuery, book: dict, db: dict):
    text = book[int(callback.data)]
    db["users"][callback.from_user.id]["page"] = int(callback.data)
    await callback.message.edit_text(
        text=text,
        reply_markup=create_pagination_keyboard(
            "forward", f"{db["users"][callback.from_user.id]["page"]}/{len(book)}", "backward"
        )
    )
